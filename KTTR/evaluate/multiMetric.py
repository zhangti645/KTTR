...
if __name__ == "__main__":
    ...
    #result_dir = '../experiment-dynamic/temporal_adaptive_2ref_512/Qstep_25_Numref_3_Threshold_0.45/evaluation/'+str(width)+'/'
    
    # result_dir = '../experiment/temporal_adaptive_2ref_512/adaptive_rgb/evaluation/'+str(width)+'/'
    
    result_dir = '../experiment/temporal_adaptive_2ref_512/Qstep_40_Numref_4_Threshold_10000000/evaluation/'+str(width)+'/'
    ...
    metric = {'psnr': 1, 'ssim': 1, 'lpips': 1, 'dists': 1}
    ...
    for seq in seqlist:
        ...
        for qp in qplist:
            ...
            f_org=open('/mnt/bolin/blchen/TestData/updata220221/orirgb/'+str(width)+'/'+str(seq)+'_'+str(width)+'x'+str(width)+'.rgb','rb')
            #f_test=open('../experiment-dynamic/temporal_adaptive_2ref_512/Qstep_25_Numref_3_Threshold_0.45/dec/'+str(width)+'/'+seq+'_QP'+qp+'.rgb','rb')

            # f_test=open('../experiment/temporal_adaptive_2ref_512/adaptive_rgb/rgb/'+str(width)+'/'+seq+'_QP'+qp+'.rgb','rb')
            
            f_test=open('../experiment/temporal_adaptive_2ref_512/Qstep_40_Numref_4_Threshold_10000000/dec/'+str(width)+'/'+seq+'_QP'+qp+'.rgb','rb')
            ...
            if metric['psnr']:
                f_psnr = open(result_dir+seq+'_QP'+str(qp)+'_psnr.txt','w')
                sum_PSNR=0
            if metric['ssim']:
                f_ssim = open(result_dir+seq+'_QP'+str(qp)+'_ssim.txt','w')
                sum_SSIM=0
            if metric['lpips']:
                f_lpips = open(result_dir+seq+'_QP'+str(qp)+'_lpips.txt','w')
                sum_LPIPS=0
            if metric['dists']:    
                f_dists = open(result_dir+seq+'_QP'+str(qp)+'_dists.txt','w')
                sum_DISTS=0
    ...
    if metric['psnr']:
        np.savetxt(result_dir+'result_psnr.txt', totalResult_PSNR, fmt = '%.5f')
    if metric['ssim']:
        np.savetxt(result_dir+'result_ssim.txt', totalResult_SSIM, fmt = '%.5f')
    if metric['lpips']:
        np.savetxt(result_dir+'result_lpips.txt', totalResult_LPIPS, fmt = '%.5f')
    if metric['dists']:
        np.savetxt(result_dir+'result_dists.txt', totalResult_DISTS, fmt = '%.5f')

...
class Logger:
    ...
    def log_scores(self,scalenumber, loss_names):  ####
        loss_mean = np.array(self.loss_list).mean(axis=0)

        loss_string = "; ".join(["%s - %.5f" % (name, value) for name, value in zip(loss_names, loss_mean)])
        loss_string = str(scalenumber)+ "-- " +str(self.epoch).zfill(self.zfill_num) + ") " + loss_string ###############        
    ...
    def visualize_rec(self, scalenumber, num_temporal, inp, out): ####
        if 'source_more' in inp:
            image = self.visualizer.visualize(num_temporal, inp, inp['source'], out, inp['source_more'])
        else:
            image = self.visualizer.visualize(num_temporal, inp, inp['source'], out)    
        imageio.imsave(os.path.join(self.visualizations_dir, "%s-%s-rec.png" % (str(scalenumber),str(self.epoch).zfill(self.zfill_num))), image) #######
    ...
    @staticmethod
    def load_cpk(scalenumber, checkpoint_path, generator=None, discriminator=None, temporaldiscriminator=None,kp_detector=None,
                 optimizer_generator=None, optimizer_discriminator=None,optimizer_temporaldiscriminator=None, optimizer_kp_detector=None):
        checkpoint = torch.load(checkpoint_path)
        if generator is not None:
            generator.load_state_dict(checkpoint['generator'+'_'+str(scalenumber)])
        if kp_detector is not None:
            kp_detector.load_state_dict(checkpoint['kp_detector'+'_'+str(scalenumber)])
        if discriminator is not None:
            try:
               discriminator.load_state_dict(checkpoint['discriminator'+'_'+str(scalenumber)])
            except:
               print ('No discriminator in the state-dict. Dicriminator will be randomly initialized')
        if temporaldiscriminator is not None:
            try:
               temporaldiscriminator.load_state_dict(checkpoint['temporaldiscriminator'+'_'+str(scalenumber)])
            except:
               print ('No temporaldiscriminator in the state-dict. temporaldiscriminator will be randomly initialized')  
        if optimizer_generator is not None:
            optimizer_generator.load_state_dict(checkpoint['optimizer_generator'+'_'+str(scalenumber)])
        if optimizer_discriminator is not None:
            try:
                optimizer_discriminator.load_state_dict(checkpoint['optimizer_discriminator'+'_'+str(scalenumber)])
            except RuntimeError as e:
                print ('No discriminator optimizer in the state-dict. Optimizer will be not initialized')     
        if optimizer_temporaldiscriminator is not None:
            try:
                optimizer_temporaldiscriminator.load_state_dict(checkpoint['optimizer_temporaldiscriminator'+'_'+str(scalenumber)])
            except RuntimeError as e:
                print ('No optimizer_temporaldiscriminator optimizer in the state-dict. Optimizer will be not initialized')      
        if optimizer_kp_detector is not None:
            optimizer_kp_detector.load_state_dict(checkpoint['optimizer_kp_detector'+'_'+str(scalenumber)])
    ...
    def log_epoch_vis(self, epoch, scalenumber, num_temporal, inp, out):
        self.epoch = epoch
        self.log_scores(scalenumber,self.names)
        self.visualize_rec(scalenumber,num_temporal, inp, out)     
        
    def log_epoch_ckp(self, epoch, scaletimes, models):
        self.epoch = epoch
        self.models = models
        self.scaletimes = scaletimes
...
class Visualizer:
    ...
    def visualize(self,num_temporal, driving_inp, source, out, source_more = None):
        images = []
        ...
        for driving_num in range(0,num_temporal) :     
            driving=driving_inp['driving_'+str(driving_num)]
        ...
        for driving_num in range(0,num_temporal) :  
            # Output image
            prediction = out['driving_'+str(driving_num)]['prediction'].data.cpu().numpy()
            prediction = np.transpose(prediction, [0, 2, 3, 1])
            images.append(prediction)

            if 'prediction_more' in out:
                prediction_more =  out['driving_'+str(driving_num)]['prediction_more'].data.cpu().numpy()
                prediction_more = np.transpose(prediction_more, [0, 2, 3, 1])
                images.append(prediction_more)

            if 'prediction_fusion' in out:
                prediction_fusion =  out['driving_'+str(driving_num)]['prediction_fusion'].data.cpu().numpy()

...
class OcclusionAwareGenerator(nn.Module):
    ...
    def __init__(self, num_channels, num_kp, num_ref, num_temporal, scale_range, block_expansion, max_features, num_down_blocks,
        ...
        if dense_motion_params is not None:
            self.dense_motion_network = DenseMotionNetwork(num_kp=num_kp, num_channels=num_channels,scale_range=scale_range,
    ...
    def forward(self, source_image, scalefactor, heatmap_source, heatmap_driving, source_image_more = None, heatmap_source_more = None):  
        
        ### resize the image into 256*256
        bs, c, h, w = source_image.shape  
        source_image_scale = F.interpolate(source_image, size=(256, 256), mode='bilinear')
        
        # Downsampling
        out = self.first(source_image_scale)
        for i in range(len(self.down_blocks)):
            out = self.down_blocks[i](out)
        
        # obtain the dense motion and occlusion map of source_image
        output_dict = {}
        dense_motion = self.dense_motion_network(scalefactor,source_image,heatmap_source,heatmap_driving)
        ...
        if source_image_more is not None:
            ...
            dense_motion_more = self.dense_motion_network(scalefactor,source_image_more,heatmap_source_more,heatmap_driving)
        ...
        out_dense = F.interpolate(out_dense, size=(w, h), mode='bilinear') ########
        ...
        if source_image_more is not None:
            ...
            out_dense_more = F.interpolate(out_dense_more, size=(w, h), mode='bilinear') ########
        ...
        if source_image_more is not None:
            ...
            out_more = F.interpolate(out_more, size=(w, h), mode='bilinear') ############
            ...
            prediction_fusion = F.interpolate(prediction_fusion, size=(w, h), mode='bilinear')   #########   #                  
